...
export default function TimetableView({ events, setEvents, selectedDate }: TimetableViewProps) {
  ...
  const TIMETABLE_COLOR_PALETTE = [
    { value: "bg-[#15253F] text-white border-[#15253F]", label: "딥 네이비", bgHex: "#15253F" },
    { value: "bg-[#354E6B] text-white border-[#354E6B]", label: "인디고 블루", bgHex: "#354E6B" },
    { value: "bg-[#566C86] text-white border-[#566C86]", label: "스틸 데님", bgHex: "#566C86" },
    { value: "bg-[#889BB0] text-white border-[#889BB0]", label: "그레이 블루", bgHex: "#889BB0" },
    { value: "bg-[#ACBFD4] text-slate-900 border-[#ACBFD4]", label: "아이스 실버", bgHex: "#ACBFD4" }
  ];

  const [selectedColor, setSelectedColor] = useState(TIMETABLE_COLOR_PALETTE[0].value);
  ...
  const handleAddEvent = () => {
    ...
    setNewTitle("");
    setErrorMsg("");
    setSelectedColor(TIMETABLE_COLOR_PALETTE[0].value);
  ...
  return (
    <div id="timetable-view-container" className="flex flex-col h-full bg-slate-50 text-slate-850">
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${isSel 
                    ? "bg-pastel-400 text-white shadow-md shadow-pastel-100" 
              ...
              {rawDayEvents.map((evt) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`
                      absolute left-2 right-2 border rounded-2xl px-3.5 py-2 flex flex-col justify-between shadow-xs select-none group focus:ring-2 focus:ring-pastel-200 transition-all cursor-pointer
                      ${evt.color}
                    `}
                    style={{
                      top: `${topPos}px`,
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full text-xs font-semibold bg-slate-100 rounded-xl px-3.5 py-2.5 focus:outline-none focus:ring-2 focus:ring-pastel-200 border-none text-slate-800 placeholder-slate-400"
                      value={newTitle}
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 block mb-2">테마 색상 선택</label>
                    <div className="flex items-center gap-3">
                      {TIMETABLE_COLOR_PALETTE.map((col) => {
                        ...
                        return (
                          <button
                            ...
                            className={`
                              w-9 h-9 rounded-xl border transition-all flex items-center justify-center cursor-pointer shadow-xs relative
                              ${isSelected ? "ring-2 ring-indigo-500 scale-105 border-transparent" : "border-slate-200 hover:scale-105"}
                            `}
                            style={{ backgroundColor: col.bgHex }}
                            title={col.label}
                          >
                            {isSelected && (
                              <Check className={`w-4 h-4 ${col.value.includes("text-white") ? "text-white" : "text-slate-800"}`} />
                            )}
                          </button>
                        );
                      })}
                  ...
                  <button 
                    ...
                    className="w-full py-3 bg-pastel-400 text-white rounded-xl text-xs font-bold hover:bg-pastel-500 shadow-md shadow-pastel-100 active:scale-95 transition-all text-center cursor-pointer block"
                  >
                    일정표에 심기
                  </button>
                </div>
      ...
      <button 
        ...
        className="fixed bottom-20 right-6 bg-pastel-305 text-slate-800 border border-pastel-250 px-5 py-3.5 rounded-full flex items-center justify-center shadow-lg shadow-pastel-100 hover:bg-pastel-400 hover:scale-105 active:scale-95 transition-all font-semibold text-xs tracking-tight gap-1.5 z-40 cursor-pointer"
